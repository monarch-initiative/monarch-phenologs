...
def generate_random_data():
    ...
    # pool.map(partial(mane.multiprocess_generate_random_data, source_organism='mouse', target_organism='zebrafish'), range(1, 1001))
    pool.map(loads(dumps(multiprocess_generate_random_data, 5)))
    print("Done!")

    return

def multiprocess_generate_random_data(i):
    # def multiprocess_generate_random_data(self, source_organism, target_organism, i):
    ...
    for j in range(len(gene_to_phenotype_df)):
        if gene_to_phenotype_df.loc[j, "phenotype"] == current_phenotype:
            ...
            d = pd.DataFrame([[gene_to_phenotype_df.loc[j, "phenotype"], ortholog['ortholog_id']]], columns=['phenotype', 'ortholog'])
...
# main.generate_random_data()


# multiprocess_generate_random_data(10)
if __name__ == "__main__":
