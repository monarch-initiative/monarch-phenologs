...
#TODO: Testing: How long does it take to process if we just generate long tables for distance and weight matrices?
# Interesting thing here: If we create a long table, we only need to do it once and then have one column for distance, one for weight...
# Can just create a table that is all pairwise combinations of phenotypes.
#

# There's probably a faster way of doing this...
for phenotype_a in phenotype_list:
    for phenotype_b in phenotype_list:

        row = (phenotype_a, phenotype_b,)
